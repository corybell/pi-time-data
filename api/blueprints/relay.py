...
@inject
def relay_list(relay_service: RelayService = Provide[Container.relay_service]):
  relays = relay_service.all()
  return jsonify(relays), 200

@blueprint.route('/relay/<string:relay_id>', methods=['GET'])
@inject
def relay_get(relay_id: str, relay_service: RelayService = Provide[Container.relay_service]):
...
@inject
def relay_edit(relay_id: str, relay_service: RelayService = Provide[Container.relay_service]):
  if relay_id not in known_relays:
    return jsonify(message='relay not found'), 404
